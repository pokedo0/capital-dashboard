...
const tooltipStyle = (
  ...
  const padding = 12;
  const rightAxisGuard = 72; // keep tooltip away from price axis so labels stay visible
  const offsetX = 70;
  const offsetY = 15;
  ...
  <div class="bg-panel border border-white/10 rounded-xl p-4 flex flex-col gap-4 w-full">
    <div class="flex flex-wrap justify-between items-center gap-4">
      ...
      <div class="flex items-center gap-3">
        <TimeRangeSelector v-model="rangeKey" :options="['1M', '3M', '6M', 'YTD', '1Y', '5Y', 'All']" />
      </div>
    </div>
    <div class="relative w-full aspect-[4/3] md:aspect-auto md:min-h-[400px] flex-1">
